...
Show the result in a nice way in a sentence: `r result`
...
Show the code and also run the code

```{r hide_run_example, echo=TRUE, results='hide',warning=FALSE}
z <- rnorm(100)
```

Show the code but do not run it

```{r hide_example, eval=FALSE}
a <- rnorm(100)
b <- 2*x + rnorm(100)
cor(a, b)
```
...
**WARING python must be installed in order to be able to run this markdown. If you do not want to install python just comment out the next block**
